...
export type RedundantPhraseType = keyof typeof redundantPhraseTypes;

// Phrase patterns to detect (using regex)
export const redundantPhrasePatterns = {
  // "Not just X, but Y" pattern
  [redundantPhraseTypes.NOT_JUST]: /not\s+just\s+([^,\.]+)(?:,|,\s+but|\s+but)\s+([^,\.]+)/gi,
  
  // "Not only is it X, but it's also Y" pattern
  [redundantPhraseTypes.NOT_ONLY]: /not\s+only\s+(?:is\s+it|are\s+they|does\s+it|do\s+they|will\s+it|can\s+it|are|is|does|do|will|can)\s+([^,\.]+)(?:,|,\s+but|\s+but)(?:\s+it\'s|\s+they\'re|\s+it|\s+they)?\s+(?:also\s+)?([^,\.]+)/gi,
  
  // "It's not just about X, it's about Y" pattern
  [redundantPhraseTypes.NOT_JUST_ABOUT]: /(?:it\'s|its|this\s+is)\s+not\s+just\s+about\s+([^,\.]+)(?:,|,\s+it\'s|\s+it\'s)\s+(?:about\s+)?([^,\.]+)/gi,
  
  // "It's not just X, it's Y" pattern
  [redundantPhraseTypes.ITS_NOT_JUST]: /(?:it\'s|its|this\s+is)\s+not\s+just\s+([^,\.]+)(?:,|,\s+it\'s|\s+it\'s)\s+([^,\.]+)/gi,
};
...
export function replaceRedundantPhrase(text: string, phraseType: RedundantPhraseType): string {
  ...
  return text.replace(pattern, (match, group1, group2) => {
    switch (phraseType) {
      case redundantPhraseTypes.NOT_JUST:
        return `${group1} and ${group2}`;
      
      case redundantPhraseTypes.NOT_ONLY:
        return `${group1} and ${group2}`;
      
      case redundantPhraseTypes.NOT_JUST_ABOUT:
        return `${group1} and ${group2} both matter`;
      
      case redundantPhraseTypes.ITS_NOT_JUST:
...
export function removeRedundantPhrases(content: string, options: PhraseRemovalOptions = {}): {
  ...
  // Apply replacements
  Object.keys(redundantPhraseTypes).forEach(type => {
    const phraseType = type as RedundantPhraseType;
