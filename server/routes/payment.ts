...
// import { PayPalGateway } from "../payment/paypal-gateway"; // Temporarily disabled for auth testing
import { db } from "@db";
...
// const paypalGateway = new PayPalGateway(); // Temporarily disabled for auth testing
...
// Test endpoint for PayPal payment gateway - temporarily disabled
router.get("/test-paypal", async (_req, res) => {
  res.json({ message: "PayPal payment testing temporarily disabled for auth validation" });
});
